...
const loginUrl = `${BASE_URL}/crearInvitado`;
let chess = new Chess();
const NombreUser = process.argv[2];
if (!NombreUser) {
    console.log("El nombre del invitado será asignado aleatoriamente.");
}

// Tercer argumento opcional para saber si hacer movimientos aleatorios o no
const randomMoves = process.argv[3] === 'rand' ? true : false; // Si es true, se hacen movimientos aleatorios
...
function buscarPartida(socket) {
    ...
    socket.on('gameOver', (gameData) => {
        ...
        // Realizar logout
        axios.post(`${BASE_URL}/borrarInvitado`, { id: userId })
            .then(() => {
...
async function main() {
    await guestLogin(NombreUser);  // Esperar a que el login se complete
}
